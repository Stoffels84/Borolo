...
def load_excels_via_ftp_three_days() -> dict[str, dict]:
    ...
    Retourneert dict per label met: filename, file_date, df (of None als niet gevonden)
    ...
    try:
        ...
        for label, d in targets.items():
            ...
            out[label] = {"filename": chosen, "file_date": extract_yyyymmdd(chosen), "df": df}
...
def render_section(label: str, payload: dict, personeelnummer_query: str):
    ...
    with c1:
        st.metric("Doeldatum", file_date.isoformat() if file_date else "—")
    with c2:
        st.metric("Bestand", filename if filename else "Niet gevonden")

    if df is None:
        st.warning(f"Geen bestand gevonden voor **{label}** (datum {file_date.isoformat()}).")
        st.divider()
        return
    ...
    if results.empty:
        st.info(f"Geen rijen gevonden voor personeelnummer **{personeelnummer_query}** in **{label}**.")
        st.divider()
        return
    ...
    # Download per sectie (optioneel)
    out = BytesIO()
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        results.to_excel(writer, index=False, sheet_name="Dienstlijst_resultaat")
    out.seek(0)

    safe_name = (filename.rsplit(".", 1)[0] if filename else label.lower())
    st.download_button(
        f"Download resultaat ({label}) als Excel",
        data=out,
        file_name=f"{safe_name}_pn_{personeelnummer_query}.xlsx",
...
def main():
    ...
    st.subheader("Deze app bevat mogelijk fouten door last minute wijzigingen - controleer zeker de uitrijlijst op GBR of E17")
    ...
    try:
        ...
        if not q.strip():
            st.info("Geef een personeelnummer in om resultaten te tonen (voor gisteren/vandaag/morgen).")
            st.stop()
